...
console.log(`mall_id: ${CAFE24API.MALL_ID}, uesr_id: ${user.member_id}, user_name: ${user.name}, item_id: ${get_item_id()}`)
...
function set_page(box){
  ...
  prev.setAttribute("class", "prev_similar");
  prev.style.fontSize = '40px'
  prev.innerHTML = "&lt;";
  next.setAttribute("class", "next_similar");
  next.innerHTML = "&gt;";
...
slide_wrapper.setAttribute("class", "slide_wrapper_similar");
slides.setAttribute("class", "slides_similar");

box.style.width = "100%"
box.style.height = "100%"
box.style.margin = 'auto'
box.setAttribute("class", "similar_box");

intellisys_box.setAttribute("class", "intellisys_similar");
intellisys_box.style.display= "flex";
title.innerHTML = "유사상품";
...
for (let i = 0; i < imglist.length; i++) {
  ...
  row.setAttribute("class", "intellisys_li");
...
var slides_ = document.querySelector('.slides_similar'),
    slide = document.querySelectorAll('.slides_similar li'),
    prevBtn = document.querySelector('.prev_similar'),
    nextBtn = document.querySelector('.next_similar'),
    currentIdx = 0,
    slideCount = slide.length,
    slideWidth = Math.ceil(slide_wrapper.offsetWidth/4 - document.querySelector('.prev_similar').offsetWidth/(imglist.length*3) *2)
...
function updateWidth(){
  var currentSlides = document.querySelectorAll('.slides_similar li')
...
function set_style() {
  ...
  return "\
  *{margin:0; padding: 0;}\
  .intellisys_li{list-style:none;}\
  .intellisys_similar{height: 200px; border: 1px solid black; margin: 0 auto;}\
  .slide_wrapper_similar{position: relative; width: 100%; height: 80%; overflow: hidden; margin: 0 auto;}\
  .slides_similar{position: absolute; left: 0; top: 0; padding-left: 0;}\
  .slides_similar.animated{transition: 0.5s ease-out;}\
  .prev_similar {margin: auto;}\
  .next_similar {margin: auto;}\
  .slides_similar li{width:"+slideWidth+"px; height: 100px; float: left;}"
}
